...
export class NeonStorage implements StorageInterface {
    ...
    async saveYjsUpdate(update: YjsUpdate): Promise<void> {
        await this.db`
    ...
    async saveSideChatMessage(message: SideChatMessage): Promise<void> {
        await this.db`
    ...
    async saveYjsDocumentState(state: YjsDocumentState): Promise<void> {
        await this.db`
    ...
    async saveSideChatThread(thread: SideChatThread): Promise<void> {
        await this.db`
    ...
    async saveActivityLog(entry: ActivityLogEntry): Promise<void> {
        await this.db`
    ...
    async getYjsUpdates(documentId: string, since?: Uint8Array): Promise<YjsUpdate[]> {
        const results = since
            ? await this.db`
    ...
    async getSuggestions(documentId: string, status?: string): Promise<Suggestion[]> {
    ...
    async getChatMessages(documentId: string, threadId?: string): Promise<ChatMessage[]> {
    ...
    async getSideChatThreads(documentId: string): Promise<SideChatThread[]> {
    ...
    async getActivityLog(documentId: string, limit?: number): Promise<ActivityLogEntry[]> {
        const results = limit
            ? await this.db`
...
export function createStorage(): StorageInterface {
    ...
    return new NeonStorage(connectionString);
}
