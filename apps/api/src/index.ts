...
initializeRoomManager(db);
const yjsManager = new YjsDocumentManager(db);
...
io.on("connection", (socket) => {
    ...
    socket.on("chat", async (data: ChatMessage) => {
        try {
            ...
            await Promise.all([roomManager.persistChatMessage(data), roomManager.persistChatMessage(aiMessage)]);
        } catch (error) {
...
// New streaming endpoint that returns the actual stream
app.post("/ai/suggest/stream", async (req, res) => {
    try {
        const { documentId, userId, prompt, context } = req.body as {
            documentId: string;
            userId: string;
